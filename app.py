...
if st.button("🚀 Run Full Analysis", type="primary"):
    with st.spinner("Analyzing all games..."):
        ...
        for idx, g in enumerate(games):
            ...
            adj = 0.0
            key = f"g{idx}"
            if st.checkbox("Hitter-Friendly Park", value=False, key=f"park_{key}"): adj += 0.10
            if st.checkbox("Warm Temp (>75°F)", value=False, key=f"temp_{key}"): adj += 0.07
            if st.checkbox("Strong Wind Out", value=False, key=f"wind_s_{key}"): adj += 0.15
            if st.checkbox("Mild Wind Out", value=False, key=f"wind_m_{key}"): adj += 0.08
            if st.checkbox("Both Lineups Full", value=True, key=f"line_{key}"): adj += 0.10
            if st.checkbox("Strong Middle Order", value=False, key=f"mid_{key}"): adj += 0.10
            if st.checkbox("Both Bullpens Thin", value=False, key=f"bull_{key}"): adj += 0.15
            ...
            edge = prob_over - 0.50  # default assumption
            col3.metric("Edge", f"{edge*100:+.1f}%", delta="✅ BET OVER" if edge > 0.05 else "PASS")
            
            with st.expander("Adjustments"):
                st.write(f"**Total Adj:** {adj:+.2f} runs")
                st.write(f"Pitching: {away_lambda:.2f} / {home_lambda:.2f}")
            
            st.divider()

st.info("✅ Each game now has its own independent checkboxes.")